...
export class SellerDashboardComponent implements OnInit {
  ...
  sellerOrderDashboard(){
    alert("this option for only VIP category");
  }

  sellerOrderDashboardData(){
    this.customerService.orderDashboarData().subscribe(data => {
      this.order_dashboard_data = data;
      console.log("Order dashboard",this.order_dashboard_data);
  ...
  sellerProductDashboardData(){
    this.customerService.productDashboardDetail().subscribe(data => {
